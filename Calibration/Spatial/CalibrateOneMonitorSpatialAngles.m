%CalibrateOneMonitorSpatialAngles
...
thisFile='CalibrateOneMonitorSpatialAngles.m';
...
sp.WhereRU      = 3; %lab PC with three Asus monitors
...
switch sp.WhereRU
    ...
    case 2 %on the lab iMac (NEC DiamondView monitor)
        gamCal='/Users/jgreenwood/Matlab/Calibration/MonitorData/CalData.mat'; %select Gamma calibration files
    ...
    case 4 %laptop
        gamCal='/Users/John/MatlabFiles/Calibration/MonitorData/CalData.mat'; %select Gamma calibration files
...
sp.ViewDist      = 57; %60 %70 %50; %viewing distance in cm
...
if RightScreen
    sp.ZeroPosX    = -round(sp.BezelPixels); %right screen
else
    sp.ZeroPosX    = round(screenRect(3) + round(sp.BezelPixels)); %left screen
end
sp.ZeroPosY       = centY;%980;%720;% 720];%[sp.ScreenResY-150 sp.ScreenResY-150];

%max eccentricity visible
sp.TotalVF      = PixToVA(sp.ViewDist,sp.PixelScale,(sp.ScreenResX+sp.BezelPixels)); %size of whole visual field
...
if RightScreen
    AngVals         = -75:5:75;%0:5:75;%-75:5:75; % [0 25 155 180];%[-45:5:75 105:5:225]; %[-60:15:60 120:15:240];
else
    AngVals         = 105:5:255;%105:5:255; % [0 25 155 180];%[-45:5:75 105:5:225]; %[-60:15:60 120:15:240];
end
...
for ang=1:numel(AngVals)
    ...
    for ecc=1:numel(EccHalf)
        ...
        EccTempY(ecc) = round(sp.ZeroPosY - sin(deg2rad(AngVals(ang))).*(EccValsPix(ecc)));
    end
