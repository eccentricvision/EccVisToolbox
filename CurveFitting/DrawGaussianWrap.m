...
%
% eg.  x=[-180:0.1:180]; prob = DrawGaussianWrap(x,-160,40,0.8,0.1); plot(x,prob); ylim([0 1]);
% eg2. x=[0:0.1:360]; prob = DrawGaussianWrap(x,10,30,0.8,0.1); plot(x,prob); ylim([0 1]);
% 
% J Greenwood 2009
...
probfull=(scaleEst.*(exp(-(xfull-uEst).^2 / (2*(varEst.^2))))); %Gaussian function (NB without offset incase this adds values from 0)
