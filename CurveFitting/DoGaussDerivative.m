function  [k t] = DoGaussDerivative(Params)
...
%maxval,sigma,offset,sigma2szeRatio,order

maxval = Params(1);
sigma  = Params(2);
offset = Params(3);
sigma2szeRatio = Params(4);
order  = 1; %fixed order for now
...
k = (k/max(k))*maxval; %convert values to -max:max
t = t + offset; %offset t values (x-axis), if required
