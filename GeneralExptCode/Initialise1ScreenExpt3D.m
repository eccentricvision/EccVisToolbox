...
if IsOSX %some hacks to make psychtoolbox work even with slightly odd timestamps
    ...
    [maxStddev, minSamples, maxDeviation, maxDuration] = Screen('Preference','SyncTestSettings',0.05,[],[]);% [, maxStddev=0.001 secs][, minSamples=50][,maxDeviation=0.1][, maxDuration=5 secs]);
end


if IsWin
    FileLoc = which('Initialise1ScreenExpt3D');
    
    if strcmp(FileLoc,'C:\Documents\MATLAB\GeneralExptCode\Initialise1ScreenExpt3D.m'); %PC in Exp Psych lab 439
        comp.machineName = 'greenwood01'; %hacky way to give the computer a machine name since this is broken in Windows 7/Vista
    elseif strcmp(FileLoc,'C:\Users\JohnG\MATLAB\GeneralExptCode\Initialise1ScreenExpt3D.m') %in the lab at Moorfields
...
if UseColour==1
    switch sp.WhereRU %1=Office, 2=Lab, 3=Laptop
        case 1 %in John's Office
            colCal='/Users/john.greenwood/Documents/MATLAB/EccVisToolbox/Calibration/MonitorData/CalDataVG278_MEH_3Dmode_RGB.mat';%OfficeCalDataRGB.mat'; %select Gamma calibration files
        case 2 %laptop
...
[w screenRect]=Screen('OpenWindow',screens(ExpScreen), 0,[],32,2,sp.StereoMode); %max(Screens)
